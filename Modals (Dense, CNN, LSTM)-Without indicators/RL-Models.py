...
class CustomAgent:
    ...
    def __init__(self, lookback_window_size=50, lr=0.00005, epochs=1, optimizer=Adam, batch_size=32, model=""):
        ...
        self.log_name = datetime.now().strftime("%Y_%m_%d")+"_Crypto_trader"+"_"+trainModelAgent
    ...
    def save(self, name="Crypto_trader", score="", args=[]):
        ...
        self.counter +=1
        self.Actor.Actor.save_weights(f"{self.log_name}/{self.counter}_{name}_Actor.h5")
        self.Critic.Critic.save_weights(f"{self.log_name}/{self.counter}_{name}_Critic.h5")
...
class CustomEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50, Render_range=100, Show_reward=False, normalize_value=65000):
...
if __name__ == "__main__":            
    ...
    trainModelAgent="CNN"
    log_name = datetime.now().strftime("%Y_%m_%d") + "_Crypto_trader" + "_"

    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=1, optimizer=Adam, batch_size = 32, model=trainModelAgent)
    train_env = CustomEnv(train_df, lookback_window_size=lookback_window_size)
    train_agent(train_env, agent, visualize=False, train_episodes=250, training_batch_size=500)

    test_env = CustomEnv(test_df, lookback_window_size=lookback_window_size, Show_reward=False)
    test_agent(test_env, agent, visualize=False, test_episodes=100, folder=log_name+"CNN", name="227_Crypto_trader", comment="")

    #agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=1, optimizer=Adam, batch_size = 128, model="LSTM")
    #test_env = CustomEnv(test_df, lookback_window_size=lookback_window_size, Show_reward=False)
    #test_agent(test_env, agent, visualize=False, test_episodes=100, folder=log_name+"LSTM", name="150_Crypto_trader", comment="")
